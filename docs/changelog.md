...
### Added
- New `stdio-auth` provider. It runs a command you supply. The command reads a JSON request on standard input and answers with JSON on standard output, so accounts can live in a file, a directory server, or the host operating system. Passwords go in on standard input only, never the command line or the environment. The contract a command must follow is in [The stdio-auth provider](plugins/stdio-auth.md).
- Two example backends under `examples/backends/`. The file backend keeps accounts in a JSON file with scrypt hashed passwords, and runs anywhere Node runs. The PAM backend uses the accounts of the Linux machine the server runs on. Reading shadow passwords needs privilege, and that privilege sits in one small setuid helper rather than in the identity provider.
- `BIND_ADDRESS` sets the interface the server listens on. It defaults to `0.0.0.0`, which is what the server did before. Name one address when a reverse proxy handles TLS, and the server stops answering on the machine's other interfaces.
...
- The theme documentation now lists the values a template must use, and what breaks without each one. We added those values and expected every template to use them without writing that down, so a theme could look right and still render a field nobody can fill in.

### Changed
- The `lru` session plugin now uses the `lru-cache` package. It was an unbounded `Map` swept every 30 seconds, so it grew until the process ran out of memory. `SESSION_LRU_MAX` sets how many entries it holds, default 10000. Eviction can end a session that has not expired, so keep that limit above the number of live sessions.

### Fixed
- The login field only took email addresses, which was fine while `simple-sql` was the only provider. `stdio-auth` uses whatever name the backend knows, and the browser would not submit one without an `@` in it. The provider now defines the field. Define nothing and it stays an email box, so `simple-sql` is unchanged. `stdio-auth` asks for a username. `LOGIN_LABEL` renames the field, whichever provider is running.
- Sign up, Reset Password and Profile led to 404 pages under every provider but `simple-sql`, the only one that serves those routes. Reset Password had no setting to turn it off at all. Providers now define which of those pages they serve, and a link disappears when nothing is behind it. Registration still needs `CLIENT_FEATURES_REGISTRATION` as well.
- A style rule joins the login form's two boxes into one control. It matched on input type, and every form shares the `form-signin` class, so it would have caught any text box added to any of them, including whatever an MFA plugin renders. The two boxes have their own names now, `login-identifier` and `login-secret`.
- The one-time passcode field's label pointed at the login form's identifier, so clicking it focused nothing.
- A repeated login field arrived as a list and a percent-encoded `%00` arrived as a real zero byte, and the provider passed both straight to the backend. The PAM example separates the two fields it sends its helper with a zero byte, so a name like `root%00hunter2` chose where that split landed and the password box was ignored. The provider now refuses a name or password that is not plain text, and the PAM example refuses one as well. A backend can still return any other character, escaped by JSON.
- A backend returning an `account_id` containing a zero byte would have put it in the `sub` claim of every token. The provider now treats that reply as a backend failure.
- The file example backend read accounts straight off a plain object, so a name of `__proto__` or `constructor` found something on the prototype instead of nothing.
- The PAM example backend refused correct passwords. Its README said to name the helper's path in the provider's settings, but the provider hands a backend an environment holding only `PATH`, so that setting never arrived. The backend fell back to the copy built beside it, which is not setuid and cannot read shadow passwords. PAM calls that an authentication failure, so a misconfigured install looked exactly like everybody mistyping.

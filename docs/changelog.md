...
### Fixed
- The login form assumed every provider identifies people by email address. Its field was `type="email"`, so the browser refused to submit a name with no `@` in it. `simple-sql` was the only provider until now and does use email addresses, so the assumption never showed. `stdio-auth` is the first that names people any other way, and nobody could have logged in with it. Providers now define what the form asks for. One that defines nothing still gets the email field, so `simple-sql` is unchanged. `stdio-auth` asks for a username, and `STDIO_AUTH_LOGIN_LABEL` changes the wording.

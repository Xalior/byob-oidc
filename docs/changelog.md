...
### Fixed
- The login field only took email addresses, which was fine while `simple-sql` was the only provider. `stdio-auth` uses whatever name the backend knows, and the browser would not submit one without an `@` in it. The provider now defines the field. Define nothing and it stays an email box, so `simple-sql` is unchanged. `stdio-auth` asks for a username. `LOGIN_LABEL` renames the field whichever provider is running, since the wording belongs to the site rather than to the provider.

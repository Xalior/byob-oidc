...
- The rule joining the login form's two boxes into one control matched on input type. Every form on the site shares the `form-signin` class, so any text input added to any of them would have been styled as half a login box. The boxes are named now, `login-identifier` and `login-secret`, which matters because an MFA plugin renders its own form with whatever inputs it likes.
- The one-time passcode field's label pointed at the login form's identifier, so clicking it focused nothing.
- The PAM example backend looked for its privileged helper next to itself, and its README said to name the path in the provider's settings. The provider gives every backend a scrubbed environment, so that setting never arrived, and the helper it found instead is not setuid and cannot read shadow passwords. PAM reported that as an authentication failure, so a misconfigured install looked exactly like everybody typing the wrong password.

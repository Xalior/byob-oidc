...
### Fixed
- Nobody could log in unless their name was an email address. The login form's field was `type="email"`, so the browser refused to submit anything without an `@` in it. Providers now say what the form should ask for. One that says nothing still gets the email field, so `simple-sql` looks the same as before. `stdio-auth` asks for a username, and `STDIO_AUTH_LOGIN_LABEL` changes the wording.
- Sign up, Reset Password and Profile led to 404 pages under every provider but `simple-sql`, the only one that serves those routes. Reset Password had no setting to turn it off at all. Providers now declare which of those pages they serve, and a link disappears when nothing is behind it. Registration still needs `CLIENT_FEATURES_REGISTRATION` as well.
